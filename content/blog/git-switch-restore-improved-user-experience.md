...
To be precise, the above sets the specified files paths to their content [^content-trees] in the _index_. If you'd like to set the files to their content in a _tree_, like a branch or a commit, instead of the index, specify it before the file paths. If it happens that the branch shares a name with the file, pass the `--` to separate the two. [^checkout-overwrites-index]
...
[^git-growth]:
  ...
  (This author bears _some_ blame for expanding the plethora of options `git checkout` takes, having [contributed the `-B` option](https://github.com/git/git/commit/02ac98374eefbe4a46d4b53a8a78057ad8ad39b7), the "forced" counterpart to `git checkout -b <branch>`.)
...
1. _When given a file path, `git checkout <filepath>` sets one or more `<filepath>` to their contents in the index_:
